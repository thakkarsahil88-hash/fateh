...
import { saveWeight, getLatestWeight } from '@/lib/storage'
      ...
      <div>
        ...
        {todayPlanDay ? (
          ...
          <div className="bg-zinc-900 rounded-2xl p-6 text-center">
            <p className="text-zinc-400">Rest day today 🛋️</p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {Array.from({ length: 14 }, (_, i) => {
            const d = new Date(todayDate); d.setDate(d.getDate() - 13 + i)
            ...
            return (
              <div key={iso} className="text-center">
                ...
                <div className={cn('w-8 h-8 mx-auto rounded-lg flex items-center justify-center text-xs',
                  workoutDates.includes(iso) ? 'bg-orange-500 text-white' : 'bg-zinc-800 text-zinc-500',
                  iso === todayDate && !workoutDates.includes(iso) && 'ring-2 ring-orange-500 ring-offset-1 ring-offset-zinc-950'
                )}>{d.getDate()}</div>
              </div>
            )
          })}
