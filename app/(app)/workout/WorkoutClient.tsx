...
import { CheckCircle, ChevronDown, ChevronUp, ExternalLink, Loader2 } from 'lucide-react'
...
export default function WorkoutClient({ phone, planDay, todayDate }: Props) {
  ...
  async function completeWorkout() {
    ...
    const { data: workoutLog, error } = await supabase
      ...
      .insert({ phone, plan_day_id: planDay?.id, date: todayDate, duration_mins: durationMins })
      .select()
      .single()
        ...
        .map((pe: any) => {
          ...
          return (
            <div key={ex.id} className="bg-zinc-900 rounded-2xl overflow-hidden">
              <button onClick={() => toggleExpand(ex.id)} className="w-full flex items-center gap-3 p-4">
                <div
                  className="w-12 h-12 rounded-xl bg-zinc-800 overflow-hidden flex-shrink-0"
                ...
                <div className="flex items-center gap-2">
                  <a href={ex.youtube_url} target="_blank" rel="noopener noreferrer"
                    onClick={e => e.stopPropagation()} className="text-zinc-500 hover:text-orange-400">
              ...
              {state.gifOpen && (
                <div className="mx-4 mb-3 rounded-xl overflow-hidden cursor-pointer" onClick={() => toggleGif(ex.id)}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="px-4 pb-4 space-y-2">
                  ...
                  {state.sets.map((s, idx) => (
                    <div key={idx} className="grid grid-cols-3 gap-2 items-center">
                      ...
                      <input type="number" min={0} step={0.5} value={s.weight_kg || ''} placeholder="0"
