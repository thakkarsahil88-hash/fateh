...
export default function WorkoutClient({ planDay, todayDate }: Props) {
  ...
  function updateSet(exId: string, idx: number, field: 'reps' | 'weight_kg', value: number) {
    setStates(prev => {
  ...
  return (
    <div className="p-5 space-y-4">
      <div className="pt-2">
        ...
        <p className="text-zinc-400 text-sm">{planExercises.length} exercises</p>
      </div>
        ...
        return (
          <div key={ex.id} className="bg-zinc-900 rounded-2xl overflow-hidden">
            <button onClick={() => toggle(ex.id, 'expanded')} className="w-full flex items-center gap-3 p-4">
              <div className="w-12 h-12 rounded-xl bg-zinc-800 overflow-hidden flex-shrink-0 cursor-pointer"
            ...
            {state.expanded && (
              <div className="px-4 pb-4 space-y-2">
                <div className="grid grid-cols-3 text-xs text-zinc-500 mb-1 px-1"><span>Set</span><span>Reps</span><span>kg</span></div>
